...
- **풀리퀘스트 보냈을때 충돌이 난다면? 두가지 방법**
	1. 현재 커밋과 병합하고 싶은 커밋을 미리 내 브랜치에서 병합해서 병합 커밋을 만들고 이를 풀 리퀘스트로 보내는 방법
		- 너구리는 1과 2의 충돌을 해결해서 만들어진 [병합커밋] 3번 커밋을 너구리 원격저장소에 올림(고양이 원본저장소는 권한이 없기에 올릴 수 x)
		- but 이렇게 먼저 병합하고 풀리퀘스트를 보내면 충돌은 나지 않겠지만 나의 풀 리퀘에서 볼 필요한 병합커밋(3번커밋)의 이력이 남는 아쉬움이 든다.
		- 변경점은 1번커밋 하나뿐인데 충돌을 해결하느라 쓸데없는 코드가 들어가게 됨
	2. **묵은 커밋을 방금 한 커밋처럼 이력 조작하는 것**
		- 너구리가 0번 커밋을 베이스로 1,2번 커밋을 만든 것을  **3번커밋**을 베이스로 만든다면. 문제업이 빨리감기 머지 상태가 됨
		- 리베이스
			: 위처럼 **커밋의 베이스를 똑 떼서 다른 곳으로 붙이는 것**
		- 내브랜치에서 내 변경사항만 남길 수 있다는 장점도 있음
		- 정리하자면 **한달 전의 코드 기준으로 만들었던 브랜치를 마치 최신 코드를 기준으로 만듣 것처럼 이력을 조작**하는 것, 만약 충돌이 있다면 이력을 조작하는 중간에 조용히 고쳐주면 됨
- 리베이스 실습
	1. [iTshirt-raccoon] 탭의 그래프를 보면 현재 커밋인 '찜하기기능추가'의 베이스 커밋이 '좋아요기능추가'인것을 확인
	2. 우리는 베이스 커밋을 마치 upstream/master의 최신 커밋인 '개발자 목록에 너구리 추가'인 것처럼 이력을 조작할 것
	3. 새롭게 베이스로 삼고싶은 커밋인 '개발자목록에 너구리 추가'를 클릭하고 [재배치] - 리베이스 한글 번역
		- 현재 있는 너구리 원격저장소의 [master]브랜치를 너구리 혼자만 사용하는 것인지 묻는 것
		- 리베이스는 히스토리를 강제로 조작하기 때문에 다른 사람이 만약 히스토리를 보고이다면 꼬임 그렇기 때문에 반드시 혼자만 쓰는 브랜치에 수행해야함
		- 보통 기능 구현하는 브랜치는 혼자만 쓰니까 편하게 리베이스
	4. 해결해야할 병합 충돌이 있다는 경고창이 뜸
	5. '커밋하지 않은 변경사항'이라는 문장이 보이고 충돌이 난 like.md 파일이 스테이지 위와 아래에 있음, 눌러보면 익숙한 충돌메세지가 보임
		
		
		<img src="../../img/detachedHead2.PNG">
		

	6.  vscode [iTshirt-raccoon]에서 충돌난 코드를 수정해줌 'Accept Both Changes'를 이용하면 자동으로 코드 수정, 수정한뒤 저장
	7.  소스트리에서 충돌이 해결된 파일은 [+] 버튼을 눌러 스테이지에 올림. 올리기전에 오른쪽 미리보기에서 충돌이 해결됐는지 꼭 확인
	8.  소스트리에서 [액션 - 재배치 계속] 을 클릭해서 리베이스 계속 진행

## chapter 5 실무 사례와 함께 Git 다루기
### 1. 실습을 위한 사전 준비 : 새로운 원격저장소 만들기
#### GitHub에서 새로운 원격저장소 만들기
- Git과 GitHub으로 관리하는 프로젝트를 시작하는 방식
```
1. GitHub에서 원격저장소를 만들고 이를 내 컴퓨터에 클론한다.
2. 내 컴퓨터에서 먼저 로컬저아소를 만들고 GitHub에 원격저장소를 만들어 로컬저장소에 원격저장소 주소를 remote add 한다.
```
- MIT라이센스
	'이 소스를 재가공해서 재배포해도 된다.'는 편의한 라이센스
-  **[remote]** 소스트리의 편의 기능을 통해 원격저장소를 클론
		소스트리에서 등록한 계정, 소스트리에서 연결한 GitHub 원격저장소가 모두 나타남

### 2. amend: 수정 못한 파일이 있어요, 방금 만든 커밋에 추가하고 싶어요
#### amend로 마지막 커밋 수정하기
- 새로운 커밋을 만들지 않고 기존에 있던 커밋을 수정하는 방법
	1.일단 수정할 파일을 원하는대로 수정
	2. 소스트리에서 추가하고 싶은 변경사항을 스테이지에 올림  
	3. **바로 커밋을 누르지 않고 우측 하단의 [커밋 옵션] 드롭다운 버튼을 누르고 [마지막 커밋 정정] 버튼**
		- 영어로는 [Amend last commit] 마지막 커밋을 수정(amend)한다는 뜻
	4.  이렇게하면 내가 지금 스테이지에 올린 변경사항이 기존 커밋에 추가되면서 덮어씌워짐
	5.  바로 전에 커밋했던 메세지가 자동완성되고 기호에 따라 커밋메세지 수정해됨
		- [커밋 옵셥] 드롭다운을 열어보면 [마지막 커밋 정정] 왼쪽에 체크 아이콘 표시
	6.  [history] 탭에서 그래도 수정된 것을 확인

#### amend로 마지막 커밋 메세지를 수정하고 원격저장소 브랜치에 강제 푸시하기
- 이번에는 이미 원격저장소에 푸시한 커밋을 amend로 수정하고 원격저장소에 다시 반영하는 실습 과정
	- [커밋옵션]에서 [마지막 커밋 정정] 체크 후 커밋메세지만 수정
	- [initial commit] 커밋을 기준으로 [origin/master] ( 원격저장소의 master브랜치)에 'amend 실습 파일 추가' 커밋이, [master] (로컬저장소의 master 브랜치)에는 'amend 실습 파일 추가 메세지 수정' 커밋이 추가됨
		- 원격저장소에 푸시하기 전까지 수정 커밋은 로컬저장소에서만 반영되어있기때문
		
			<img src="../../img/amend.PNG">
	
	- 로컬저장소의 변경사항을 원격저장소에 강제로 덮어씌우며 푸시하는 명령은 [강제푸시]
	- [강제푸시]는 나 혼자만 쓰고 있는 브랜치에서만 해야함
	- 공용으로 사용하는 브랜치에서 했을 경우에 히스토리가 엉망이 됨
	- 그렇기때문에 옵션은 기본적으로 꺼져있음
	- [ 도구 - 옵션 - Git] 탭에서 [강제 푸시 가능] 체크박스 선택하고 [push] - [강제푸시] 체크 
	- 그래프가 깔끔해짐
	
		<img src="../../img/amend2.PNG"> 
...
|브랜치명|특징|
|------|---|
|feat/기능이름| 1. 각 개발자가 개발중인 브랜치 2. 직접 커밋을 올린다|
|master|1.feat/기능이름 브랜치에서 개발 완료된 코드가 합쳐진 브랜치 2.출시전인 베타 버전이라고 볼 수 있다. 3. 직접 커밋을 올리지 않는다(병합을 통해서만 코드를 업데이트)|
|latest|1.실제 출시할 코드(대중에게 보여줄 완벽한 코드)를 올리는 브랜치 2. [master] 브랜치에서 굵직한 개발이 끝나면 출시 시점에 [latest] 브랜치로 코드를 병합|

- 어제 출시한 코드가 담긴 [latest]브랜치에서 당장 고쳐야 하는 버그가 이다는 것을 뒤늦게 알게된 상황
	1. 문어가 급하게 [master] 브랜치에서 [fix/text-bug]브랜치를 따서 고치고 이를 [master] 브랜치에 병합
	2. 이제 이를 [latest] 브랜치에 반영해야하는데 [master] 브랜치에 있는 **다른 변경사항 말고 딱 그 버그를 고친 커밋만 반영하려고 한다면 ? 체리픽 기능 사용**
		- 내가 따길 원하는 체리(커밋)을 선택하고 [체리픽]버튼을 클릭

#### cherry-pick:다른 브랜치의 커밋 하나만 내 브랜치에 반영하기
**이번 실습은 [feat/a]브랜치, [feat/b] 브랜치를 만들고 [feat/a]브랜치의 커밋하나를 [feat/b] 브랜치에 반영할 계획**
1. [feat/a] 브랜치 생성
	- 커밋 세개 추가
2. master브랜치에 돌아가서  [feat/b] 브랜치 생성  후 커밋
3. a브랜치와 b브랜치를 고양이와 문어가 각각 나눠서 개발하고 있다는 전제하에, 문어는 **a 브랜치에서 '두번째커밋' 만 가져와서 b브랜치에 병합하고 싶은 상황**
	- 두번째 커밋만 가져오고 싶은데 a 브랜치와 병합하면 첫번째, 세번째 커밋도 b브랜치에 반영되니 
	- 이런 상황에서 **체리픽**이 사용됨
4. b브랜치에 있는 상태에서 복제를 원하는 두번째 커밋 오른쪽 버튼 [체리픽] 선택
5. 이제 두번째 커밋이 b 브랜치에 반영됨. cherrypick.md  파일이 추가된 것
6.  성공적으로 두번째 커밋이 b 브랜치 최신 커밋 위에 달라붙은 것을 볼 수 있음
	- **but a브랜치의 두번째 커밋 ID와 b브랜치 두번째 커밋 ID는 다름** 
	- **변경사항을 복사해왔지만 서로 같은 커밋은 아니라는 뜻!**
	
		<img src="../../img/cherrypick.PNG">

### 4. reset: 옛날 커밋으로 브랜치를 되돌리고 싶어요
#### Soft/Mixed reset: 모든 기억을 남기면서 브랜치 되돌리기
1. 현재 b 브랜치는 '두번째 커밋 - 탐나는 커밋' 에 있음 최신 상태를 하나 전 커밋인 'featb 기능 추가' 커밋으로 돌려야함
	- featb에서 오르쪽 마우스 [이 커밋까지 현재 브랜치를 초기화]
2. [이 커밋까지 현재 브랜치를 초기화] 옵션  - 기본 옵션인 [Mixed] 선택
	- [soft] [Mixed] 는 원하는 커밋으로 이력을 되돌리긴 하지만, 이다음에 추가했던 모든 변경사항을 작업 공간으로 뽑아줌
	
		<img src="../../img/mixedmode.PNG">

	- **'커밋하지 않은 변경사항' 이 생김, 두번째 커밋에서 추가했던 cherrypick.md 변경사항이 스테이지 아래로 튕겨져 나옴**
	- **시간은 성공적으로 되돌렸지만 변경사항은 '두번째커밋 - 탐나는 커밋'을 커밋하기 전 상태로 남겨둔 것**
	- **즉 커밋을 없던걸로 되돌렸지만 파일 기록이 남아있으니 내 마음대로 다시 수정할 수 있음 변수를 줄일 수 있는 장점이 있다.**
	- [soft] [Mixed] 와 차이점
		- 비슷하지만 soft가 mixed와 다른 점은 변경사항이 스테이지 위에 있는지 아래에 있는지의 차이!  
		- soft - 바로 커밋할 수 있는 특징, mixed는 다시 무엇을 스테이지 위로 add할지 고민 할 수 있음


#### Hard reset: 모든 기억을 지우며 브랜치를 되돌리기
1. [Hard] 모드는 내가 어디에 있든지, 커밋하지 않은 변경사항이 얼마나 있는지와 상관없이 깔끔하게 이동
	- 두번째 커밋 - 탐나는 커밋을 [이 커밋까지 현재 브랜치를 초기화]시킴
	
	<img src="../../img/hardmode1.PNG">
		
	작업하지 않은 변경사항도 모두 날아감 

2.  **'featb 기능 추가' 과거로 돌아가서 두번째 커밋이 마치 없었던 일인 것처럼 만들기**

	<img src="../../img/hardmode2.PNG">
	
	브랜치가 깔끔하게 과거로 돌아감
	
4. 하지만 아직 원격브랜치인 [origin/feat/b]에는 두번째커밋이 남아있음
	- 히스토리를 수정하는 푸시이기 때문에 [강제푸시] 해야함
5.  원격브랜치 [origin/fear/b]의 이력도 성공적으로 과거로 돌아감

	<img src="../../img/hardmode3.PNG">

### 5. revert: 이 커밋의 변경사항을 되돌리고 싶어요
- reset :  커밋을 마치 없었던 일로 되돌리는 방법
- 하지만 함께 쓰는 브랜치라면 **변경사항을 되돌리는 새로운 커밋을 만드는 것이 더 좋다.**
-  revert : '사이트 제목 삭제' 커밋의 변경사항을 없애고 싶을때 '사이트 제목 삭제의 변경사항 되돌리기' 라는 새로운 커밋을 추가 
#### revert:커밋의 변경사항을 되돌리는 새로운 커밋 만들기
- 히스토리 패널에서 되돌리고 싶은 커밋 - [커밋 되돌리기]
	- '사이트 제목 삭제' 를 명시적으로 되돌리는 커밋인 'Revert 사이트 제목 삭제' 커밋이 만들어짐
	- revert는 방금한 커밋뿐만 아니라 잘못된 커밋이 있으면 언제들지 revert 할 수 있음

### 6. stash: 변경사항을 잠시 다른 곳에 저장하고 싶어요, 커밋은 안 만들래요
#### stash : 커밋하지 않은 변경 사항을 서랍 속에 넣어두기
- 스태시 

	변경 사항을 잠깐 서랍속에 넣어뒀다가 다시 꺼내 쓰는 방법
- 스태시 메세지 입력하고 자유롭게 pull 이나 브랜치 이동
	- **stash에는 tracked 상태(추적중 - 한번이라도 Git에 올렸던 상태)인 파일들만 들어간다.**
